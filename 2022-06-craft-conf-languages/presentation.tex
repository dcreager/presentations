...
\begin{frame}[fragile]
    ...
    \showlisting<1>[firstline=5,lastline=22]{80mm - 10em}{10mm}{20em}{go}{examples/download.go}
    \showlisting<2>[firstline=5,lastline=10]{80mm - 10em}{10mm}{20em}{go}{examples/download-goroutine.go}
    ...
    \only<2>{\pdfpcnote{
        ...
        creating a new "goroutine" for each download. Each goroutine is a block
        of code, and Go's runtime takes care of scheduling goroutines
        efficiently across however many CPUs are available. Importantly, if any
        goroutine is blocked - waiting for a response from another machine, for
        instance - the Go scheduler automatically moves it aside to allow other
        goroutines to proceed. \\
        \\
        This is what the concurrent version looks like, by performing each
        Download call within a separate goroutine. \\
        \\
    ...
    \only<3>{\pdfpcnote{
        Go provides several helper types in its standard library to handle these
        kinds of situation. In particular, we can use a "WaitGroup" to wait for
        all three goroutines to finish before returning. Because the downloads
        happen in goroutines, they still get to run concurrently.
    }}
\end{frame}
...
\begin{frame}[fragile]
    ...
    \showlisting<1>[firstline=6,lastline=11]{80mm - 10em}{10mm}{20em}{python}{examples/download-tasks.py}
    \showlisting<2>[firstline=8,lastline=15]{80mm - 10em}{10mm}{20em}{rust}{examples/download.rs}
    \showlisting<3>[firstline=1,lastline=7]{80mm - 10em}{10mm}{20em}{javascript}{examples/download.js}

    \only<1>{\pdfpcnote{
        async: Go solution requires tight coupling between language, stdlib, and
        runtime scheduler. OS thread solution can be inefficient with > 10,000
        threads. Async makes it clearer in the code where the potential
        "blocking" points are. Also allows scheduler to be just another library
        component. Same syntax to enqueue tasks in e.g. separate work pools vs
