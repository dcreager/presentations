...
\tikzset{
    envframe/.style={
        ...
        fill=white,
        row sep=-0.5em,
        every outer matrix/.append style={inner xsep=0.2em,inner ysep=0em},
        column 1/.style={
            anchor=base west,
        },
        column 2/.style={
...
\begin{frame}[fragile]
    ...
    \begin{tikzpicture}
        \matrix[envframe] (mod) {
            ...
            \only<2- >{
                \node[frame name]{math}; \&
                \node[frame special]{math module};
            }
            \only<4- >{ \\
                \node[frame name]{pythagoras}; \&
                \node[frame special]{pythagoras function};
            }
            \\
        };
        \only<3- >{\matrix[envframe,above=of mod]{
            \& \node{$\cdots$}; \\
            \node[frame name]{sqrt}; \&
