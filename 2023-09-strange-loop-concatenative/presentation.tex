...
\newcommand\compiled[2][bgcolor=lightgray,minipage=20mm]{%
    \adjustbox{#1,margin=2pt}{\ttfamily\smaller[3]\begin{tabular}{@{~}l@{~}}#2\end{tabular}}%
    \ignorespaces
}
...
\begin{frame}
    \flattitle{10mm}{10mm}{Associativity}
    todo: insert picture
\end{frame}
...
\begin{frame}[fragile]
    ...
    \begin{equation*}
        \only<.(7)->{\left.}
        \begin{array}{c@{\;\,}c@{\,\;}c}
            \only<.>{\llbracket p_1 ~ p_2 ~ p_3 ~ p_4 \rrbracket}
            \only<.(1)->{\llbracket (p_1 ~ p_2) ~ (p_3 ~ p_4)) \rrbracket}
            &=&
            \only<.-.(1)>{\qqq}
            \only<.(2)>{
                \llbracket p_3 ~ p_4 \rrbracket \circ
                \llbracket p_2 ~ p_1 \rrbracket
            }
            \only<.(3)->{
                (\llbracket p_4 \rrbracket \circ \llbracket p_3 \rrbracket) \circ
                (\llbracket p_2 \rrbracket \circ \llbracket p_1 \rrbracket)
            }
            \only<.(4)->{
                \\
                \llbracket p_1 ~ (p_2 ~ (p_3 ~ p_4)) \rrbracket &=&
                ((\llbracket p_4 \rrbracket \circ \llbracket p_3 \rrbracket) \circ
                \llbracket p_2 \rrbracket) \circ \llbracket p_1 \rrbracket
            }
            \only<.(5)->{
    ...
    \begin{onlyenv}<+->
        \vskip 2em
        \begin{center}
            \textbf{\textcolor{cyan}{Associativity $\Rightarrow$ parallelism}}
        \end{center}
    \end{onlyenv}%
    \citepaper{
        Manfred von Thun.
