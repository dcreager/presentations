...
\begin{frame}[fragile]
    ...
    \begin{column}{0.3\textwidth}
        \begin{minted}[autogobble,frame=single,framesep=6pt,label=a.py,escapeinside=??]{python}
            def broil?\tikzmark{ex1broildef}?():
                pass

            def fry():
                pass

            broil?\tikzmark{ex1broilref}?()
        \end{minted}
        \uncover<1>  {\curvecodeline{[yshift=8pt] pic cs:ex1broilref}{+(25:2cm) and +(-45:1cm)}{[yshift=-4pt] pic cs:ex1broildef}}
        \uncover<2- >{\codeline{pic cs:ex1broilref}{pic cs:ex1broildef}}
    \end{column}
