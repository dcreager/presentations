...
\newcommand{\symbolstack}[1]{{\scriptsize Symbol stack:} $\langle {\small \texttt{#1}} \rangle$}
...
\begin{frame}[fragile]
    ...
    \uncover<2,4>{\highlightdef{def}}
    \uncover<3,4>{\highlightboth{import}}
    \uncover<3,4>{\highlightref{ref}}
    ...
    \begin{center}
        \uncover<3,4>{
            ...
            might refer to \texttt{stove.broil} in some other file
        }
        \\
        \uncover<4>{+}
        \\
    ...
    \end{minipage}
    \end{center}
    \uncover<3,4>{\curvecodeline%
      [-]%
      {[yshift=10pt] pic cs:ref_end}%
      {+(45:1cm) and +(-155:1cm)}%
      {[yshift=-4pt] pic cs:import_start}%
    }
    \uncover<4>{\curvecodeline%
      {[xshift=12pt, yshift=10pt] pic cs:import_start}%
      {+(90:2cm) and +(75:2cm)}%
      {[xshift=-6pt, yshift=10pt] pic cs:def_end}%
    }
...
\begin{frame}[fragile]
    ...
    \begin{tikzpicture}[baseline={(current bounding box.center)}]
        ...
        \node (broil def)  [highlight node on=<2>]
...
\begin{frame}[fragile]
    ...
    \begin{tikzpicture}[baseline={([yshift=2em] current bounding box.center)}]
        ...
        \node (broil ref)  [highlight node on=<2>]
                           [reference]   [right=of k2]            {broil};
        \node (k3)         [anon scope]  [above=\scopegap of k2]  {};
        \node (import def) [highlight node on=<3>]
                           [pop]         [right=of k3]            {broil};
        \node (import ref) [highlight node on=<3>]
                           [push]        [right=of import def]    {broil};
        \node (import dot) [highlight node on=<3>]
                           [push]        [right=of import ref]    {.};
        \node (import mod) [highlight node on=<3>]
...
\begin{frame}[fragile]
    ...
    \scalebox{0.75}{
    \begin{tikzpicture}
        \node (root)       [highlight node on=<11>]
                           [root node]                            {};
        \node (stove)      [highlight node on=<12>]
                           [definition]  [left=of root]           {stove};
        \node (dot)        [highlight node on=<13>]
                           [pop]         [left=of stove]          {.};
        \node (s1)         [highlight node on=<14>]
                           [anon scope]  [left=of dot]            {};

        \setwidestsymbol{broil}
        \node (s2)         [highlight node on=<15>]
                           [anon scope]  [above=\scopegap of s1]  {};
        \node (saute def)  [definition]  [left=of s2]             {\sized{saute}};
        \node (s3)         [highlight node on=<16>]
                           [anon scope]  [above=\scopegap of s2]  {};
        \node (broil def)  [highlight node on=<17>]
                           [definition]  [left=of s3]             {\sized{broil}};
        \node (s4)         [anon scope]  [above=\scopegap of s3]  {};
        \node (bake def)   [definition]  [left=of s4]             {\sized{bake}};
        \node (s5)         [anon scope]  [above=\scopegap of s4]  {};
        ...
        \path [-Stealth]
          (root)      edge [highlight inner edge on=<2>, show path on=<12>] (stove)
          (stove)     edge [highlight inner edge on=<2>, show path on=<13>] (dot)
          (dot)       edge [highlight inner edge on=<2>, show path on=<14>] (s1)
          (s1)        edge [highlight inner edge on=<2>, show path on=<15>] (s2)
          (s2)        edge [highlight inner edge on=<2>, show path on=<16>] (s3)
                      edge (saute def)
          (s3)        edge (s4)
                      edge [highlight end edge on=<2>, show path on=<17>]   (broil def)
          (s4)        edge (s5)
                      edge (bake def)
          ;
        ...
        \node (k2)         [highlight node on=<5>]
                           [anon scope]  [above=\scopegap of k1]  {};
        \node (broil ref)  [highlight node on=<4>]
                           [reference]   [right=of k2]            {broil};
        \node (k3)         [highlight node on=<6>]
                           [anon scope]  [above=\scopegap of k2]  {};
        \node (import def) [highlight node on=<7>]
                           [pop]         [right=of k3]            {broil};
        \node (import ref) [highlight node on=<8>]
                           [push]        [right=of import def]    {broil};
        \node (import dot) [highlight node on=<9>]
                           [push]        [right=of import ref]    {.};
        \node (import mod) [highlight node on=<10>]
                           [push]        [right=of import dot]    {stove};
        \node (k4)         [anon scope]  [above=\scopegap of k3]  {};
        ...
        \path [-Stealth]
          ...
          (dot)        edge (k1)
          (k1)         edge (k2)
          (broil ref)  edge [highlight inner edge on=<2>, show path on=<5>] (k2)
          (k2)         edge [highlight inner edge on=<2>, show path on=<6>] (k3)
          (k3)         edge (k4)
                       edge [highlight inner edge on=<2>, show path on=<7>] (import def)
          (import def) edge [highlight inner edge on=<2>, show path on=<8>] (import ref)
          (import ref) edge [highlight inner edge on=<2>, show path on=<9>] (import dot)
          (import dot) edge [highlight inner edge on=<2>, show path on=<10>] (import mod)
          (import mod) edge [overlay, in control=+(-60:3cm), out control=+(-90:4cm)]
                            [highlight inner edge on=<2>, show path on=<11>] (root)
          ;
    \end{tikzpicture}
    }
    \vskip 2em
    ...
    \begin{overprint}
        \onslide<4-6,8,13-16> \symbolstack{broil}
        \onslide<3,7,17>      \symbolstack{}
        \onslide<9,12>        \symbolstack{.broil}
        \onslide<10-11>       \symbolstack{stove.broil}
    \end{overprint}
    \end{minipage}
    \end{center}
\end{frame}
