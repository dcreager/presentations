...
\begin{frame}[fragile]
    ...
    \begin{tikzpicture}
        ...
        \node (instance)   [highlight node on={<2,34>}]
                           [pop]  [left=of call]                  {.};
        \node (m1)         [highlight node on={<2,35>}]
                           [anon scope]  [left=of instance]       {};

        \setwidestsymbol{broil}
        \node (m2)         [highlight node on={<2,36>}]
                           [anon scope]  [above=\scopegap of m1]  {};
        \node (saute def)  [highlight node on={<2>}]
                           [definition]  [left=of m2]             {\sized{saute}};
        \node (m3)         [highlight node on={<2,37>}]
                           [anon scope]  [above=\scopegap of m2]  {};
        \node (broil def)  [highlight node on={<2,5,38>}]
        ...
        \path [-Stealth]
          ...
          (instance)  edge [highlight inner edge on=<5>, show path on=<35>] (m1)
          (m1)        edge [highlight inner edge on=<5>, show path on=<36>] (m2)
          (m2)        edge [highlight inner edge on=<5>, show path on=<37>] (m3)
                      edge (saute def)
          (m3)        edge (m4)
                      edge [highlight end edge on=<5>, show path on=<38>]   (broil def)
          (m4)        edge (m5)
                      edge (bake def)
          ;
    ...
    \begin{minipage}{0.5\textwidth}
    \begin{overprint}
        \onslide<6,38>                  \currentsymbolstack{}
        \onslide<7,34-37>               \currentsymbolstack{broil}
        \onslide<8,12,33>               \currentsymbolstack{.broil}
